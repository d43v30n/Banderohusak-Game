...
class EnemyClass:
    ...
    def create_enemy(self):
        fenemy = pygame.transform.scale(pygame.image.load(
            'enemy.png').convert_alpha(), (SIZE_ENEMY))
        fenemy_rect = pygame.Rect(WIDTH, random.randint(
            (HEIGHT / 4), (3*HEIGHT/4)), *SIZE_ENEMY)
...
class BonusClass:
    ...
    def create_bonus(self):
        bonus = pygame.transform.scale(pygame.image.load(
            'bonus.png').convert_alpha(), (SIZE_BONUS))
        bonus_rect = pygame.Rect(random.randint(
            (WIDTH / 4), (3*WIDTH/4)), 0, *SIZE_BONUS)
